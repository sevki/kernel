...
static void snapshot_bio_end_io(struct bio *bio)
{
	...
	if (bio->bi_status) {
		pr_alert_ratelimited("snapshot io error sector %llu block error %d\n",
...
{
	struct bio *bio = bio_alloc(batch->bdev, 1, GFP_NOIO | __GFP_HIGH, op | op_flags);
...
static int snapshot_rw_block_device(int direction, struct snapshot_data *data)
{
	...
	if (!res) {
		swsusp_show_speed(start, ktime_get(),
				  sector >> PAGE_SECTORS_SHIFT,
				  "transferred image via ioctl");
	}
